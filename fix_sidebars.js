...
function updateSidebars() {
    ...
    const match = adminHtml.match(/<aside class="sidebar" id="sidebar">[\s\S]*?<\/aside>/);
    if (!match) {
    ...
    acceptedHtml = acceptedHtml.replace(/<aside class="sidebar" id="sidebar">[\s\S]*?<\/aside>/, adminSidebar);
    ...
    inwardipHtml = inwardipHtml.replace(/<aside class="sidebar" id="sidebar">[\s\S]*?<\/aside>/, adminSidebar);
    ...
    inwardipHtml = inwardipHtml.replace(/<h2>Models &amp; Parts Catalog<\/h2>/, '<h2>Models &amp; Parts Catalog</h2>');
