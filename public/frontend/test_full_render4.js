...
const document = {
    getElementById: (id) => {
        return { style: {}, classList: { add:()=>{}, remove:()=>{}, toggle:()=>{} }, innerHTML: '', addEventListener: ()=>{}, value: '' };
    }
};
...
entries = require('fs').readFileSync('c:/Users/raamp/OneDrive/Desktop/iqc/entries.json', 'utf8');
entries = JSON.parse(entries);

try {
    renderTable();
    console.log('RenderTable finished successfully.');
    console.log('HTML Length:', tableBody.innerHTML.length);
    console.log(tableBody.innerHTML.substring(tableBody.innerHTML.length - 100));
} catch (e) {
